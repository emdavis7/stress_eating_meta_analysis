...
bhl_meta %>% filter((mismatch_flag==F & follow_grp2 == 1)) %>% nrow() 
...
first <- rma.mv(final_hedges_g, var_hedges_g, 
                ...
                subset =(mismatch_flag==F & follow_grp2 == 1),
                slab = full_id)
...
#### Pub bias check #####
png(filename="images/funnel_sens_tp1_1.png", width=1200, height=1200, units = "px",
    family="sans",
    res = 200)
funnel(first, xlab = "Hedges' g", 
       digits = 2)
title(main = "Funnel Plot of Immediate 
...
####### Forest plot ###########################################################
png(filename="images/forest_plot_imm_postint.png", width=1500, height=3100, units = "px",
    family="sans",
    res = 200)
forest(first_rob,
       order = "fit",
       shade = T)
title(main = "Forest Plot of Immediate Post-Intervention Effects")
dev.off()
