...
bhl_meta %>% filter((mismatch_flag==F & follow_grp == 1)) %>% nrow() 
...
#### Pub bias check #####
png(filename="images/funnel_short.png", width=1200, height=1200, units = "px",
    family="sans",
    res = 200)
funnel(first, xlab = "Hedges' g", 
       digits = 2)
title(main = "Funnel Plot of Effects up to 3 Months")
dev.off()
...
####### Forest plot ###########################################################
png(filename="images/forest_plot_first.png", width=1500, height=3100, units = "px",
    family="sans",
    res = 200)
forest(first_rob,
       order = "fit",
       shade = T)
title(main = "Forest Plot of Short-Term Effects (Less than 3 Months)")
dev.off()
